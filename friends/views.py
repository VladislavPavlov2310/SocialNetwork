...
class FindFriendsListView(ListView):
    ...
    def get_queryset(self):
        ...
        users = User.objects.exclude(id__in=current_user_friends).\
            exclude(id__in=sent_request).exclude(id=self.request.user.id)
...
def send_request(request, username=None):
    if username is not None:
        ...
        notification = CustomNotification.objects.\
            create(type="friend", recipient=friend_user, actor=request.user, verb="sent you friend request")
